...
import { AnyDto, GeoLocationsInput, ListOpts, MediaKind, PlacementsInput, TargetingSpec } from './types/ads.type'
import {
...
export class FacebookAdsService {
    ...
    if (targeting.flexible_spec) {
      targeting.flexible_spec = targeting.flexible_spec
        .map((fs) => {
          ...
          return {
            ...fs,
            interests: fs.interests.filter((i) => !/film|movie|show|penguin|cartoon/i.test(i.name)),
          }
        })
  ...
  `
    ...
    try {
      ...
      this.logger.log(
        `[FacebookAdsService] ✅ Lookup result interests: ${JSON.stringify(lookedUp.map((x) => ({ id: x.id, name: x.name })))}`,
      )
  ...
  private async createAdSetWithPerfGoalAndDestination(
    ...
    const makeRequest = (tp: any, goal: string, campId: string) => {
      ...
      return fb.post(`/${adAccountId}/adsets`, qs.stringify(body), {
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      })
    }
