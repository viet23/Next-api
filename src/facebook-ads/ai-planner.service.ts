...
export class AiPlannerService {
    ...
    async suggestPlanFromReport(report: any, currentTargeting: any, campaignObjective?: string) {
        const systemPrompt =
            `Bạn là trợ lý tối ưu quảng cáo Meta. Chỉ trả JSON theo schema:
{
  ...
  "ab_test": {
    ...
    "variants": [{"name": string,"primaryText": string,"imageHash": string?}]?
        ...
        const userPrompt =
            `Mục tiêu campaign: ${campaignObjective || 'UNKNOWN'}
...
Yêu cầu: chỉ trả JSON hợp lệ theo schema; nếu không chắc thì bỏ field.`
        ...
        // Sanitize nhẹ
        if (plan?.genders) plan.genders = (Array.isArray(plan.genders) ? plan.genders : []).filter((g: any) => g === 1 || g === 2)
