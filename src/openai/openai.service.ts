...
export class OpenaiService {
  ...
  async analyzeTargeting(prompt: string, user: User) {
    ...
    if (
      !topcamData ||
      !topcamData.topCam ||
      topcamData?.updatedAt < todayStart ||
      topcamData?.topCam?.length === 0 ||
      topcamData?.topCam?.top3Campaigns?.length === 0 ||
      topcamData?.topCam?.items?.length === 0
    ) {
      const config = {
        apiVersion: 'v19.0',
...
Không trả thêm bất kỳ ký tự nào khác.`,
  ...
  async rewriteText(prompt: string) {
    ...
    try {
      ...
      const newText = data?.choices?.[0]?.message?.content?.trim() || '【Không tạo được nội dung gợi ý】'
  ...
  async generateText(prompt: any) {
    ...
    try {
      ...
      const text = data?.choices?.[0]?.message?.content?.trim() || '【Không tạo được nội dung gợi ý】'
  ...
  async simpleChat(prompt: string) {
    ...
    try {
      ...
      const text = data?.choices?.[0]?.message?.content?.trim() || '【Không tạo được phản hồi】'
  ...
  async translateAndExpandPrompt(text: string) {
    ...
    try {
      ...
      const newPrompt = data?.choices?.[0]?.message?.content?.trim() || '【Không tạo được prompt】'
  ...
  async generateCaptionFromDescription(contentGenerateCaption: string, description: string) {
    ...
    try {
      ...
      const caption = data?.choices?.[0]?.message?.content?.trim() || '【Không tạo được caption】'
  ...
  async chatWithPrompt(promptContent: string) {
    ...
    try {
      ...
      const msg =
        err.response?.data?.error?.message || err.response?.data?.message || err.message || 'OpenAI request failed'
  ...
  private handleOpenAiError(err: any, funcName: string, meta?: Record<string, any>): never {
    ...
    if (status === 408 || code === 'ECONNABORTED')
      throw new BadRequestException('OpenAI timeout, thử giảm yêu cầu hoặc tăng timeout')
