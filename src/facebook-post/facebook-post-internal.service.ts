...
export class FacebookPostIInternalService {
  ...
  private readonly lookupIPv4 = (hostname: string, options: any, callback: any) => {
    if (typeof options === 'function') {
      callback = options as any
      options = {}
    }
    return dns.lookup(hostname, { ...(options as dns.LookupOneOptions), family: 4 }, callback as any)
  }
    ...
    try {
      ...
      const isDeprecateAttachments =
        ...
        (fbErr?.message || '')
          .toString()
          .toLowerCase()
          .includes('deprecate_post_aggregated_fields_for_attachement'.toLowerCase())
    ...
    const worker = async () => {
      while (queue.length) {
        ...
        try {
          ...
          this.logger.warn(`Reach fail for ${id}: ${e?.message || (e instanceof Error ? e.message : String(e))}`)
          out.set(id, 0)
        }
      }
    }
  ...
  async fetchPagePostsForUser(userdto: User) {
    ...
    this.logger.log(
      ...
      user ? { id: user.id, email: user.email, idPage: user.idPage } : null,
    )
    ...
    const formatted = posts.map((post: any, idx: number) => {
      const reactionsTotal = post?.reactions?.summary?.total_count ?? post?.likes?.summary?.total_count ?? 0
  ...
  private async fetchPageViewsDaily(
    ...
    try {
      if (tryPresetFirst) {
        ...
        const { data } = await this.withRetry(() => this.fbHttp.get(baseUrl, { params: paramsPreset, headers }))
      ...
      const { data: dataRange } = await this.withRetry(() => this.fbHttp.get(baseUrl, { params: paramsRange, headers }))
